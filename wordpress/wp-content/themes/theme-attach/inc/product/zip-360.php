...
function product_360_get_attachment_id($value)
{
    ...
    if (is_string($value)) return (int)attachment_url_to_postid($value);
...
function product_360_unpack_zip($zip_id, $post_id)
{
    ...
    // Asegura permisos (Linux)
    @chmod($p['root_dir'], 0755);
    @chmod($p['target_dir'], 0755);

    // 1) Intento con unzip_file (WP)
    ...
    // 2) Fallback con ZipArchive (por si en Linux unzip_file falla por FS_METHOD/credenciales)
    if (is_wp_error($result)) {
        product_360_log('unzip_file falló, probando ZipArchive', $result->get_error_message());

        if (!class_exists('ZipArchive')) {
            product_360_log('ZipArchive NO disponible en servidor (php-zip)', null);
            return false;
        }

        $zip = new ZipArchive();
        if ($zip->open($p['zip_path']) !== true) {
            product_360_log('ZipArchive no pudo abrir el zip', $p['zip_path']);
            return false;
        }

        $ok = $zip->extractTo($p['target_dir']);
        $zip->close();

        if (!$ok) {
            product_360_log('ZipArchive extractTo falló', $p['target_dir']);
            return false;
        }
    }
    ...
    // Orden + renombrado estable a frame-N.ext
    natsort($files);
    $files = array_values($files);

    $ext = strtolower(pathinfo($files[0], PATHINFO_EXTENSION)) ?: 'png';
    $amount = count($files);
...
function product_360_get_data($zip_id, $post_id)
{
    ...
    $amount = count($files);
    $ext    = pathinfo(basename($files[0]), PATHINFO_EXTENSION) ?: 'png';
